...
class EventConnection {
    ...
    _bind_events(host: string, port: number, callback?: (err?: Error) => void): void {
        ...
        function reportTerminalError(err: Error) {
            ...
            if (self._socket && !self._socket.destroyed) self._socket.destroy();
        }
            ...
            try {
                ...
                while (pos < buf.length) {
                            ...
                            if (self.eventcallback)
                                self.eventcallback(null, { eventid: event_id, counts: tmp_event });
                            break;
                        default:
                ...
                if (err instanceof RangeError) { // incomplete packet case
                    xdr.buffer = xdr.buffer.slice(op_pos);
                    xdr.pos = 0;
                    self._xdr = xdr;
                } else {
